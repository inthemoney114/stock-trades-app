...
if "trades" not in st.session_state:
    st.session_state.trades = pd.DataFrame(columns=["Stock","Action","Quantity","Price","Brokerage","Remaining"])
...
with st.form("trade_form"):
    ...
    if submitted:
        ...
        new_trade = pd.DataFrame({"Stock":[stock],
                                  "Action":[action],
                                  "Quantity":[qty],
                                  "Price":[price],
                                  "Brokerage":[brokerage],
...
for stock, trades in st.session_state.trades.groupby("Stock"):
    ...
    portfolio_data.append({
        ...
        "Current Price": 0.0,  # could fetch live price later
        "Unrealized P/L": 0.0  # will calculate if current price provided
    })
...
total_invested = sum(portfolio_df["Quantity"] * portfolio_df["Average Cost"])
total_value = sum(portfolio_df["Quantity"] * portfolio_df["Current Price"])
